...
export const Utility = {
  ...
  getUserToken() {
    // Check if the environment variable is defined and has a valid string value
    if (process.env.REACT_APP_ENCRYPTION_KEY && typeof process.env.REACT_APP_ENCRYPTION_KEY === 'string') {
      const encryptedToken = localStorage.getItem("access_token") || "";
      try {
        const decryptedToken = AES.decrypt(encryptedToken, process.env.REACT_APP_ENCRYPTION_KEY);
        if (decryptedToken) {
          return decryptedToken.toString(enc.Utf8) || "";
        } else {
          console.error("Error decoding user token: Decryption failed");
        }
      } catch (error) {
  ...
  // refreshTokenProcess() {
  //   RequestAPI.postRequest(Api.tokenRefresh, "", { "refreshToken": Utility.getRefreshToken() }, {}, async (res: any) => {
  //     const { status, body } = res;
  //     isFetch = true
  //     if (status === 200) {
  //       if (body.error && body.error.message) {
